...
public class OAuthAuthenticator {
    ...
    func saveState(_ authState: OIDAuthState?, _ appAuthStateKey: String) throws {
    ...
    func loadState(_ appAuthStateKey: String) throws -> OIDAuthState? {
...
extension OAuthAuthenticator: Authenticator {
    ...
    public func authenticate() async throws -> String {
        ...
        if let authState = try loadState(authStateKey), authState.refreshToken != nil {
                        ...
                        if let error = error as? NSError {
                            if error.code == -10, let callbackController = self.callbackController {
                                Task {
                                    ...
                                    guard let accessToken = self._authState?.lastTokenResponse?.accessToken else {
            ...
            guard let accessToken = _authState?.lastTokenResponse?.accessToken else {
                throw OAuthAuthenticatorError.tokenMissing
            }
    ...
    public func logout() async throws {
        ...
        let _ = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<OIDEndSessionResponse, Error>) in
            DispatchQueue.main.async { [weak self] in
                ...
                if let callbackController = self?.rootViewController() {
                    ...
                    do {
                        try self?.authState(from: nil)
                    } catch {
                        continuation.resume(throwing: error)
                    }
                } else {
                    continuation.resume(throwing: OAuthAuthenticatorError.invalidState)
                }
            }
        }
